...
import { Search, ChevronRight, X, Layers, Filter } from 'lucide-react-native';
...
// Fallback category hierarchy if API returns basic categories without nested subcategories
...
export const CategoriesScreen: React.FC = () => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (res && Array.isArray(res.data) && res.data.length > 0) {
        // Merge API data with FALLBACK subcategories if API data lacks nested subcategories
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      // Fallback: If API returned empty list for selectedCat, fetch all products and filter client-side
      if (list.length === 0) {
      ...
      // Soft client-side subcategory filtering
      ...
      // Soft client-side sub-subcategory filtering
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.chipSection}>
        <Text style={styles.sectionHeaderLabel}>CATEGORIES</Text>
      ...
      {availableSubcats.length > 0 && (
        <View style={styles.subcatSection}>
          <Text style={styles.sectionHeaderLabel}>
            SUBCATEGORIES IN {selectedCatObj?.name.toUpperCase()}
          </Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16 }}>
            <TouchableOpacity
      ...
      {availableSubSubcats.length > 0 && (
        <View style={styles.subSubcatSection}>
          <Text style={styles.sectionHeaderLabel}>
            OPTION / SUB-SUBCATEGORY FOR {selectedSubcat.toUpperCase()}
          </Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16 }}>
            <TouchableOpacity
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyTitle}>No Silver Products Found</Text>
              <Text style={styles.emptyDesc}>
                No items match the selected subcategory or sub-subcategory option. Try resetting your filter choices.
              </Text>
              <TouchableOpacity style={styles.resetButton} onPress={clearAllFilters}>
                <Text style={styles.resetButtonText}>View All Collections</Text>
              </TouchableOpacity>
            </View>
          }
        />
